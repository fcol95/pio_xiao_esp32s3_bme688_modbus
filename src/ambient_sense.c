...
void ambient_sense_task(void *pvParameter) {
  ...
  ret_esp = modbus_params_set_holding_register_uint(AMBIENT_SENSE_PERIOD_MS,
                                                    task_period_ms);
  if (ret_esp != ESP_OK) {
    ESP_LOGE(LOG_TAG,
             "Failed to set modbus parameter AMBIENT_SENSE_PERIOD_MS!");
  }
  ...
  while (1) {
    ...
    vTaskDelay(pdMS_TO_TICKS(
        1 + (bme68x_get_meas_dur(BME68X_FORCED_MODE, &conf, &bme688_handle) /
             1000)));
    ...
    if (ret == BME68X_OK && n_fields > 0) {
      ESP_LOGV(LOG_TAG,
               "Temperature: %.1f°C, Pressure: %.1fhPa, Humidity: %.1f%%, Gas "
               "Resistance: %.2fMOhms.",
    ...
    ret_esp = modbus_params_set_input_register_float(AMBIENT_TEMP_DEGC,
                                                     data.temperature);
    if (ret_esp != ESP_OK) {
      ESP_LOGE(LOG_TAG,
               "Failed to updated modbus parameter AMBIENT_TEMP_DEGC!");
    }

    ret_esp = modbus_params_set_input_register_float(AMBIENT_PRESSURE_HPA,
                                                     data.pressure / 100.0f);
    if (ret_esp != ESP_OK) {
      ESP_LOGE(LOG_TAG,
               "Failed to updated modbus parameter AMBIENT_PRESSURE_HPA!");
    }

    ret_esp =
        modbus_params_set_input_register_float(AMBIENT_HUMI_PCT, data.humidity);
    if (ret_esp != ESP_OK) {
      ESP_LOGE(LOG_TAG, "Failed to updated modbus parameter AMBIENT_HUMI_PCT!");
    }

    uint16_t new_task_period_ms;
    ret_esp = modbus_params_get_holding_register_uint(AMBIENT_SENSE_PERIOD_MS,
                                                      &new_task_period_ms);
    if (ret_esp != ESP_OK) {
      ESP_LOGE(LOG_TAG,
               "Failed to get modbus parameter AMBIENT_SENSE_PERIOD_MS!");
    } else if (new_task_period_ms != 0 &&
               task_period_ms != new_task_period_ms) {
      ESP_LOGI(LOG_TAG, "Measurement period updated from %d to %dms!",
               task_period_ms, new_task_period_ms);
      task_period_ms = new_task_period_ms;
    }
